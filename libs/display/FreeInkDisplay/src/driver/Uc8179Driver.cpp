...
bool Uc8179Driver::displayStart(EpdBus& bus, const uint8_t* fb, const uint8_t* prev, RefreshMode mode, bool turnOff) {
  ...
  // branches rewrite it a few lines down. Settle the debt here, before _grayBase
  // is repurposed as this frame's snapshot.
  if (_dtm1Stale) {
    if (mode == RefreshMode::Fast && !_needFullClear && _oldPlaneValid) {
      flushPendingOldPlane(bus);
    } else {
      _dtm1Stale = false;
    }
